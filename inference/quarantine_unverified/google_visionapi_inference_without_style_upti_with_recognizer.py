...
class Diffusion:
    ...
    def sampling(self, model, vae, n, x_text, labels, args, style_extractor, noise_scheduler, mix_rate=None, cfg_scale=3, transform=None, character_classes=None, tokenizer=None, text_encoder=None, run_idx=None):
        ...
        model.train()
        recognized_texts = recognize_urdu_batch_google_vision(x)  # Updated to Google Vision
        print(f"Generated texts recognized: {recognized_texts}")
        return x

# New Google Vision API-based recognizer (replaces Tesseract)
def recognize_urdu_batch_google_vision(image_tensors):
    client = vision.ImageAnnotatorClient()  # Assumes credentials are set
    recognized_texts = []
    for img_tensor in image_tensors:
        ...
        # Convert to bytes
        buffered = BytesIO()
        img.save(buffered, format="PNG")  # Or JPEG if preferred
        ...
        response = client.text_detection(
            image=image,
            image_context={"language_hints": ["ur"]}  # Urdu hint
        )
...
def validate(diffusion, model, vae, test_loader, num_classes, noise_scheduler, transforms, args, tokenizer=None, text_encoder=None, inception_feat=None, inception_prob=None):
    ...
    with torch.no_grad():
        for i, data in enumerate(test_loader):
            ...
            recognized = recognize_urdu_batch_google_vision(sampled_images)  # Updated to Google Vision
